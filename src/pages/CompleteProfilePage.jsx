...
const CompleteProfilePage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 4. Récupérer les données du questionnaire depuis demandes_prospects
      ...
      try {
        ...
        if (!error && data) {
          ...
          // Mettre à jour le prospect avec l'organisation_id
          await supabase
            .from('demandes_prospects')
        ...
        console.error('Erreur récupération prospect:', e);
      }

      // 5. Rafraîchir les données utilisateur
      await refreshUserData();

      // 6. Rediriger vers la démo avec les données du questionnaire
      navigate('/demo', {
        state: {
          request: {
            organisation_id: orgData.id,
            domaines_demandes: prospectData?.domaines_demandes || ['ssi'],
